...
import com.wiqer.holes.util.EasyFastUnsafe;
...
public class EasyFastSkipListMap<K,V> extends AbstractMap<K,V>
     ...
     */
    protected Node<K,V> findNode(Object key) {
        if (key == null)
            throw new NullPointerException(); // don't postpone errors
    ...
    private Node<K,V> doGetNextNode(Object key) {
        if (key == null)
            throw new NullPointerException();
        Comparator<? super K> cmp = comparator;
        outer: for (;;) {
    ...
    public  Node<K,V> put(Node<K,V> currentTailNode, K key, V value) {
        if (value == null)
            throw new NullPointerException();
        return doPutTail(currentTailNode,key, value, false);
    }
    private  Node<K,V> doPutTail( Node<K,V> currentTailNode, K key, V value, boolean onlyIfAbsent) {
        Node<K,V> z;             // added node
        if (key == null)
            throw new NullPointerException();
        Comparator<? super K> cmp = comparator;
        outer: for (;;) {
            for (Node<K,V> b = (currentTailNode==null?head.node:currentTailNode), n = b.next;;) {
                if (n != null) {
                    Object v; int c;
                    Node<K,V> f = n.next;
                    if (n != b.next)               // inconsistent read
    ...
    public  Node<K,V>  getNextNode(Object key) {
